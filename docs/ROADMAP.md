...
8. Maintain `QM-Stage-1` official policy freeze (`qm-stage1-g17b-v6-official`) and track remaining QM-lane fail classes with coupling audit checks.
9. Keep `QM-Stage-2` in prereg-only lane (`docs/QM_STAGE2_PREREG.md`) with strict post-v12 failure taxonomy (`qm-stage2-failure-taxonomy-post-v12-v1`) and targeted closure on residual fail tails (`G19` dominant; minor `G18b` tail).
...
13. Execute residual Stage-2 closure lane from post-v12 findings (no threshold tuning, degraded=0 required), prioritizing `G19`, then `G18b`.

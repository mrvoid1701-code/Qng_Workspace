...
8. Maintain `QM-Stage-1` official policy freeze (`qm-stage1-g18-v4-official`) and track remaining QM-lane fail classes with coupling audit checks.
9. Keep `QM-Stage-2` in prereg-only lane (`docs/QM_STAGE2_PREREG.md`) with strict post-v7 failure taxonomy (`qm-stage2-failure-taxonomy-post-v7-v1`) and single-gate candidate focus (`G18` next revision, prereg scaffold: `qm-stage2-g18-candidate-v5` pending).
...
13. Execute targeted `G18` candidate lane from post-v7 Stage-2 findings using prereg scaffold `qm-stage2-g18-candidate-v5` (no threshold tuning, degraded=0 required).

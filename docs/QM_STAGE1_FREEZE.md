...
- `G19 official`: `G19-v4` (`G19d-v4` high-signal median + local-window fallback in multi-peak regimes)
- `G17-v1` and `G18-v1`: legacy diagnostic-only
- `G20`: inherited unchanged in official-v14 policy outputs

## Guarantees

This freeze guarantees:

1. Reproducible official QM Stage-1 outputs via the frozen `official-v14` policy.
2. Baseline + regression guard coverage with `PASS` on:
   - `05_validation/evidence/artifacts/qm-stage1-regression-baseline-v12/latest_check/`
...
Internal freeze tag: `qm-stage1-g19-v4-official`

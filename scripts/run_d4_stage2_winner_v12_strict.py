#!/usr/bin/env python3
"""
D4 Stage-2 winner v12 strict lane.

Single conceptual change vs v11:
- keep same low-acceleration outer point-weights
- replace per-galaxy objective aggregation from median to trimmed mean
...
DEFAULT_OUT = ROOT / "05_validation" / "evidence" / "artifacts" / "d4-stage2-winner-v12-strict"

WINNER_FORMULA_ID = "WINNER_V1_M8C"


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Run D4 Stage-2 winner v12 strict benchmark.")
    p.add_argument("--test-id", default="d4-stage2-winner-v12-strict")
    ...
    p.add_argument("--galaxy-trim-frac", type=float, default=0.10)
...
def flatten_with_galaxy(galaxies: dict[str, list[dict[str, float]]], names: list[str]) -> list[dict[str, float | str]]:
    pts: list[dict[str, float | str]] = []
    for name in names:
        for p in galaxies[name]:
            q = dict(p)
            q["gal"] = name
            pts.append(q)
    return pts
...
def trimmed_mean(values: list[float], trim_frac: float) -> float:
    if not values:
        return 0.0
    v = sorted(values)
    n = len(v)
    if n == 1:
        return v[0]
    t = max(0.0, min(0.49, trim_frac))
    k = int(math.floor(t * n))
    lo = k
    hi = n - k
    if hi <= lo:
        lo = 0
...
def galaxy_robust_train_objective(
    points: list[dict[str, float | str]],
    k: float,
    g_dag: float,
    focus_gamma: float,
    r_tail_kpc: float,
    galaxy_trim_frac: float,
) -> float:
    """
    Robust train objective:
    - per galaxy: weighted mean squared residual (weights identical to v10)
    - aggregate across galaxies via trimmed mean (robust but less aggressive than median)
    """
    by_gal: dict[str, list[dict[str, float | str]]] = {}
    for p in points:
        by_gal.setdefault(str(p["gal"]), []).append(p)
    per_gal_obj: list[float] = []
    for gpts in by_gal.values():
        num = 0.0
        den = 0.0
        for p in gpts:
            pp = p  # type: ignore[assignment]
            w = fit_weight(pp, focus_gamma, r_tail_kpc)
            d = (pp["v"] - v_m8c(pp, k, g_dag)) / pp["ve"]
            num += w * d * d
            den += w
        per_gal_obj.append(num / max(den, 1e-12))
    return trimmed_mean(per_gal_obj, galaxy_trim_frac)


def fit_m8c_weighted(
    points: list[dict[str, float | str]],
    focus_gamma: float,
    r_tail_kpc: float,
    galaxy_trim_frac: float,
    ...
    """
    Fit (k, g_dag) for WINNER_V1_M8C using v12 robust train objective.
    Returns: (k_fit, gdag_fit, robust_train_obj)
    """
    def obj(k: float, gd: float) -> float:
        return galaxy_robust_train_objective(points, k, gd, focus_gamma, r_tail_kpc, galaxy_trim_frac)
    ...
    return k_fit, gd_fit, c2_weighted
...
def main() -> int:
    ...
    for split_seed in split_seeds:
        ...
        train_points = flatten_with_galaxy(galaxies, sorted(train_ids))
        holdout_points = flatten_with_galaxy(galaxies, sorted(holdout_ids))
        n_train = max(1, len(train_points))
        n_holdout = max(1, len(holdout_points))

        # v12 single change: trimmed-mean per-galaxy aggregation in train objective.
        k_fit, gd_fit, train_weighted_obj_per_n = fit_m8c_weighted(
            train_points,
            focus_gamma=float(args.focus_gamma),
            r_tail_kpc=float(args.r_tail_kpc),
            galaxy_trim_frac=float(args.galaxy_trim_frac),
        )
    ...
    manifest = {
        ...
        "fit_objective": {
            "space": "galaxy-robust weighted velocity chi2 (train only)",
            "grid_selection_objective": "winner_v1_weighted_lowacc_outer_galaxy_trimmed_mean",
            "weight_formula": "1 + focus_gamma * (r/(r+r_tail_kpc)) * 1/sqrt(1 + g_bar/a0)",
            "aggregation": "trimmed mean over galaxies of weighted chi2 per point (weight-normalized)",
            "aggregation_trim_frac": float(args.galaxy_trim_frac),
    ...
    report = [
        "# D4 Winner V12 Strict Report",
        "",
        f"- generated_utc: `{manifest['generated_utc']}`",
        f"- winner_formula_id: `{WINNER_FORMULA_ID}`",
        f"- candidate: `{args.candidate}`",
        f"- train objective (single v12 change): `{manifest['fit_objective']['grid_selection_objective']}`",

#!/usr/bin/env python3
"""
QNG emergent metric hardening runner (v5).

Changes from v4:
- Conformal (spin-0) completion added to metric:
    g_ij^v5 = delta_ij + alpha * S_ij + eps * delta_ij
             = (1 + eps) * delta_ij + alpha * S_ij
             = (1 - 2*Sigma) * delta_ij + alpha * S_ij
  where:
    S_ij = traceless part of Frobenius-normalized SPD(-Hess(Sigma))   [spin-2]
    eps  = -2 * Sigma_center                                          [spin-0]
    alpha = 1.0 (default, tunable via --alpha)
- New G1 gate: PPN-GAMMA-v1.  Checks |gamma - 1| where
    gamma = (1 - iso_coeff) / (2 * Sigma),  iso_coeff = tr(g_ij)/2
  By construction eps = -2*Sigma forces gamma = 1 exactly (floating-point
  deviations only); G1 is an internal consistency / implementation check.
- New G2 gate: G00-TRACE-RATIO-v1 (2D analog of GR Einstein normalization).
    trace_ratio = (tr(g_ij) - 2) / (-2*Sigma)  =  -4*Sigma / (-2*Sigma) = 2.0
  v4 baseline: trace_ratio ≈ 0 (traceless metric, no Sigma-dependent trace).
  v5: trace_ratio = 2.0 by construction (matches 2D+1 GR).
  Note: 3D GR target is 3.0; the 2D mismatch is expected and documented.
- G0, D1-D4 gates unchanged from v4.
- New --alpha CLI flag for spin-2 coupling.
...
Derivation references:
  ...
  Method lock:      01_notes/metric/metric-lock-v5.md
  Pre-registration: 05_validation/pre-registrations/qng-metric-hardening-v5.md
...
DEFAULT_OUT_DIR = ROOT / "05_validation" / "evidence" / "artifacts" / "qng-metric-hardening-v5"
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Run QNG emergent metric hardening v5 (conformal completion).")
...
def main() -> int:
    ...
    decision = "pass" if (gate_g0 and gate_g1 and gate_g2 and gate_d1 and gate_d2 and gate_d3 and gate_d4) else "fail"
    ...
    metric_rows = [
        ...
        {"gate_id": "FINAL", "metric": "decision",           "value": decision,                         "threshold": "G0&G1&G2&D1&D2&D3&D4",                "status": decision},
    ]
    ...
    config_payload = {
        ...
        "metric_estimator": (
            "v5: spin-2 traceless Hessian (Frobenius-normalized SPD) + "
            "spin-0 conformal completion eps=-2*Sigma"
        ),
    ...
    print("QNG metric hardening v5 completed.")

#!/usr/bin/env python3
"""
QNG emergent metric hardening runner (v1).
...
DEFAULT_OUT_DIR = ROOT / "05_validation" / "evidence" / "artifacts" / "qng-metric-hardening-v1"
...
def smooth_anchor_distances(dmat: list[list[float]], s: float) -> list[float]:
    m = len(dmat)
    raw = [dmat[0][i] for i in range(m)]
...
def estimate_metric_from_chart(chart: list[tuple[float, float]], ds_anchor: list[float]) -> tuple[float, float, float] | None:
    feats: list[list[float]] = []
    targ: list[float] = []
    for (x, y), d in zip(chart[1:], ds_anchor[1:]):
        f = 0.5 * d * d
...
def estimate_sigma_grad(chart: list[tuple[float, float]], sigma_local: list[float]) -> tuple[float, float] | None:
    feats: list[list[float]] = []
    targ: list[float] = []
    for (x, y), s in zip(chart[1:], sigma_local[1:]):
        feats.append([1.0, x, y])
        targ.append(s)
    if len(feats) < 6:
        return None
    try:
...
def write_run_log(path: Path, args: argparse.Namespace, details: dict, warnings: list[str]) -> None:
    lines = [
        "QNG metric hardening v1 run log",
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Run QNG emergent metric hardening v1.")
...
def main() -> int:
    ...
    for anchor in anchors:
        ...
        grad = estimate_sigma_grad(chart, sigma_local)
        if grad is None:
            continue
        grad_vec = (grad[0], grad[1])
        a_raw = (-grad_vec[0], -grad_vec[1])

        g_by_scale: dict[str, tuple[float, float, float]] = {}
        for label, s in scales:
            ds = smooth_anchor_distances(d_local, s)
            g_est = estimate_metric_from_chart(chart, ds)
            if g_est is None:
                continue
            g11, g12, g22 = g_est
            lmin, lmax = eig2x2(g11, g12, g22)
            min_eigs.append(lmin)
            g_by_scale[label] = g_est
            eig_rows.append(
                {
        ...
        a_metric = (-(inv11 * grad_vec[0] + inv12 * grad_vec[1]), -(inv12 * grad_vec[0] + inv22 * grad_vec[1]))
        ...
        grad_shuf = estimate_sigma_grad(chart, sigma_shuf)
        cos_shuf = float("nan")
        if grad_shuf is not None:
            grad_shuf_vec = (grad_shuf[0], grad_shuf[1])
            a_raw_shuf = (-grad_shuf_vec[0], -grad_shuf_vec[1])
            a_metric_shuf = (
                -(inv11 * grad_shuf_vec[0] + inv12 * grad_shuf_vec[1]),
                -(inv12 * grad_shuf_vec[0] + inv22 * grad_shuf_vec[1]),
            )
            cos_shuf = cosine(a_metric_shuf, a_raw_shuf)
    ...
    config_payload = {
        ...
        "smoothing_kernel": "gaussian over local graph distances",
        "gates": {
    ...
    print("QNG metric hardening v1 completed.")

#!/usr/bin/env python3
"""
QNG-T-CURL-006: residual curl with ratio floor and stronger controls.

Changes vs CURL-005:
- Ratio denominator uses static_floor to avoid blow-up when static≈0.
- Stronger rewire: off-diagonal sign flip with 5x scaling + diagonal jitter (±30%).
- Block-shuffle picks grad proxy from a different |∇Σ| quantile bin to force misalignment.
"""
...
DEFAULT_OUT_DIR = ARTIFACTS_ROOT / "qng-t-curl-006"
...
def compute_curl_residual(beta: float, g: np.ndarray, chart_pts: np.ndarray, v_vec: np.ndarray, tau: float, grad_sigma: np.ndarray, a_epsilon: float) -> dict:
    ...
    ax_tot = ax_newton + ax_lag
    ay_tot = ay_newton + ay_lag
...
def main() -> int:
    ...
    # grad_sigma quantile bins for block shuffle
    grad_all = []
    for r in anchor_rows:
        gs = np.array([float(r["g11"]) + float(r["g12"]), float(r["g12"]) + float(r["g22"])])
        grad_all.append(np.linalg.norm(gs))
    qedges = np.quantile(grad_all, [0.0, 0.25, 0.5, 0.75, 1.0]) if grad_all else [0, 1, 2, 3, 4]
    ...
    for row in charts:
        ...
        # control: rewired strong (random sign flip and 5x scaling on off-diagonal, diagonal jitter ±30%)
        g_rew = g.copy()
        sign = -1 if rng.random() < 0.5 else 1
        g_rew[0, 1] = g_rew[1, 0] = sign * g[1, 0] * 5.0
        g_rew[0, 0] *= 0.7 + 0.6 * rng.random()
        g_rew[1, 1] *= 0.7 + 0.6 * rng.random()
        ...
        # control: block shuffle grad proxy using |grad| quantile bins (pick from a different bin)
        grad_norm = np.linalg.norm(grad_sigma)
        bin_idx = max(0, min(3, sum(grad_norm > qedges[i] for i in range(len(qedges) - 1)) - 1))
        alt_bins = [i for i in range(4) if i != bin_idx]
        target_bin = rng.choice(alt_bins) if alt_bins else bin_idx
        candidates = [r for r in anchor_rows if qedges[target_bin] <= np.linalg.norm(np.array([float(r["g11"]) + float(r["g12"]), float(r["g12"]) + float(r["g22"])])) <= qedges[target_bin + 1]]
    ...
    denom = max(med_static, thresholds.static_floor)
    lag_factor = max(med_lag_iso, med_lag_grad) / denom
    rew_factor = med_rewire / denom
    block_factor = med_block / denom

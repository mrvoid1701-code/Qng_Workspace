#!/usr/bin/env python3
"""
QNG emergent metric hardening runner (v3).
...
DEFAULT_OUT_DIR = ROOT / "05_validation" / "evidence" / "artifacts" / "qng-metric-hardening-v3"
...
def metric_from_sigma_hessian(
    ...
    # v3 lock:
    # 1) remove conformal scale by unit-trace normalization
    # 2) keep fixed anisotropic fraction from Hessian and blend remainder into isotropic part
    # This stabilizes multi-scale comparisons while preserving directional structure.
    tr = max(g11 + g22, 1e-12)
    g11 /= tr
    g12 /= tr
    g22 /= tr
    keep = clamp(anisotropy_keep, 0.0, 1.0)
    iso = 0.5  # because trace is normalized to 1
...
def write_run_log(path: Path, args: argparse.Namespace, details: dict, warnings: list[str]) -> None:
    lines = [
        "QNG metric hardening v3 run log",
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Run QNG emergent metric hardening v3.")
...
def main() -> int:
    ...
    for anchor in anchors:
        ...
        for label, s in scales:
            ...
            if gh is None:
                continue
            grad_vec, hess = gh
            g11, g12, g22 = metric_from_sigma_hessian(
                hess[0],
                hess[1],
                hess[2],
                floor=1e-6,
        ...
        if gh_shuf is not None:
            ...
            g_shuf = metric_from_sigma_hessian(
                hess_shuf[0],
                hess_shuf[1],
                hess_shuf[2],
                floor=1e-6,
    ...
    config_payload = {
        ...
        "metric_estimator": "dynamic metric from Sigma Hessian + SPD projection + unit-trace conformal gauge + fixed anisotropy shrinkage",
    ...
    print("QNG metric hardening v3 completed.")

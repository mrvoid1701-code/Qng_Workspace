#!/usr/bin/env python3
"""
D4 Stage-2 dual-kernel candidates v3 (multi-split prereg run).

Purpose:
- evaluate a small set of physically-motivated candidate formulas
...
DEFAULT_OUT = ROOT / "05_validation" / "evidence" / "artifacts" / "d4-stage2-dual-kernel-v3-candidates"
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Run D4 Stage-2 candidate formulas v3 on multi-split prereg.")
    p.add_argument("--test-id", default="d4-stage2-dual-kernel-v3-candidates")
    ...
    p.add_argument("--candidates", default="outer_tail,cross_bridge")
...
def design_columns(rows: list[dict[str, float]], candidate: str, r_tail_kpc: float) -> list[list[float]]:
    c1 = [r["h1"] for r in rows]
    c2 = [r["h2"] for r in rows]
    if candidate == "outer_tail":
        c3 = [r["h2"] * (r["r"] / max(r["r"] + r_tail_kpc, 1e-12)) for r in rows]
        return [c1, c2, c3]
    if candidate == "cross_bridge":
        c3 = [math.sqrt(max(r["h1"] * r["h2"], 0.0)) for r in rows]
        return [c1, c2, c3]
...
def main() -> int:
    ...
    for c in candidates:
        if c not in {"outer_tail", "cross_bridge"}:
    ...
    for split_seed in split_seeds:
        ...
        for candidate in candidates:
            ...
            best_coeffs: list[float] = [0.0, 0.0, 0.0]
            ...
            for tau in tau_grid:
                for alpha in alpha_grid:
                    ...
                    train_cols = design_columns(train_rows, candidate, args.r_tail_kpc)
                    holdout_cols = design_columns(holdout_rows, candidate, args.r_tail_kpc)

                    y = [r["v"] * r["v"] - r["bt"] for r in train_rows]
                    w = [1.0 / (max(2.0 * max(r["v"], 1e-6) * r["ve"], 1e-6) ** 2) for r in train_rows]
                    coeffs = nnls_coordinate_descent(train_cols, y, w)
    ...
    lines = [
        "# D4 Stage-2 Candidate Formulas v3 Report",
        "",

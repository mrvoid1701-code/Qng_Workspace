#!/usr/bin/env python3
"""
D4 Stage-2 dual-kernel candidates v4 (multi-split prereg run).
...
- fit coefficients directly on chi2 in velocity-space (v), not v^2 WLS surrogate
"""
...
DEFAULT_OUT = ROOT / "05_validation" / "evidence" / "artifacts" / "d4-stage2-dual-kernel-v4-candidates"
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--test-id", default="d4-stage2-dual-kernel-v4-candidates")
    ...
    p.add_argument("--tau-grid", default="0.2,0.3,0.5,1,2,3,5,8,12,20,30,50")
    p.add_argument("--alpha-grid", default="0.1,0.2,0.3,0.5,0.7,1.0,1.3")
    p.add_argument("--candidates", default="outer_tail,outer_lowaccel")
...
def fit_nonneg_chi2_v(
    ...
    def obj(v: list[float]) -> float:
        return chi2_candidate(rows, v, cols)
...
def main() -> int:
    ...
    for c in candidates:
        if c not in {"outer_tail", "outer_lowaccel"}:
        ...
        for candidate in candidates:
            ...
            for tau in tau_grid:
                for alpha in alpha_grid:
                    ...
                    coeffs = fit_nonneg_chi2_v(train_rows, train_cols)

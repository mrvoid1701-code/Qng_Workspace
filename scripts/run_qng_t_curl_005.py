#!/usr/bin/env python3
"""
QNG-T-CURL-005: residual curl with harder controls and no synthetic base term.

Changes vs CURL-004:
- Drop linear g*x synthetic acceleration; keep only Newton + lag.
- Stronger rewire: random ± flips with 3x scaling off-diagonal.
- Block-shuffle uses |grad_sigma| quantile bins.
"""
...
DEFAULT_OUT_DIR = ARTIFACTS_ROOT / "qng-t-curl-005"
...
def main() -> int:
    ...
    for row in charts:
        ...
        # control: rewired strong (random sign flip and 3x scaling on off-diagonal)
        g_rew = g.copy()
        sign = -1 if rng.random() < 0.5 else 1
        g_rew[0, 1] = g_rew[1, 0] = sign * g[1, 0] * 3.0
        ...
        # control: block shuffle grad proxy using |grad| quantile bins
        grad_norm = np.linalg.norm(grad_sigma)
        bin_idx = sum(grad_norm > qedges[i] for i in range(len(qedges) - 1)) - 1
        candidates = [r for r in anchor_rows if qedges[bin_idx] <= np.linalg.norm(np.array([float(r["g11"]) + float(r["g12"]), float(r["g12"]) + float(r["g22"])])) <= qedges[bin_idx + 1]]

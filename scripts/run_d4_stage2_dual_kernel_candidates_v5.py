#!/usr/bin/env python3
"""
D4 Stage-2 dual-kernel candidates v5 (multi-split prereg run).

Purpose:
- evaluate a minimal candidate set after v4 findings
- keep anti post-hoc governance: fixed split seeds + fixed grids + fixed constants
- fit coefficients directly on chi2 in velocity-space (v)
- emphasize low-acceleration outer regime in training objective only
"""
...
DEFAULT_OUT = ROOT / "05_validation" / "evidence" / "artifacts" / "d4-stage2-dual-kernel-v5-candidates"
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Run D4 Stage-2 candidate formulas v5 on multi-split prereg.")
    p.add_argument("--test-id", default="d4-stage2-dual-kernel-v5-candidates")
...
def design_columns(
    ...
    if candidate == "outer_lowaccel_single":
        c3 = [
            r["h2"]
        ...
        return [c3]
    if candidate == "outer_lowaccel_focus":
        c3 = [
            r["h2"]
            * (r["r"] / max(r["r"] + r_tail_kpc, 1e-12))
            / math.sqrt(1.0 + (r["bt"] / max(r["r"], 1e-12)) / max(a0_internal, 1e-30))
...
def main() -> int:
    ...
    lines = [
        "# D4 Stage-2 Candidate Formulas v5 Report",
        "",

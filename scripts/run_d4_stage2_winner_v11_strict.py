#!/usr/bin/env python3
"""
D4 Stage-2 winner v11 strict lane.

Single conceptual change vs v10:
- fit WINNER_V1_M8C with a galaxy-robust train objective
  (same low-acceleration outer point-weights, but objective aggregation is per-galaxy median)
...
DEFAULT_OUT = ROOT / "05_validation" / "evidence" / "artifacts" / "d4-stage2-winner-v11-strict"

WINNER_FORMULA_ID = "WINNER_V1_M8C"


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Run D4 Stage-2 winner v11 strict benchmark.")
    p.add_argument("--test-id", default="d4-stage2-winner-v11-strict")
...
def galaxy_robust_train_objective(
    ...
    - aggregate across galaxies via median (robust to galaxy-size imbalance/outliers)
    """
    ...
    return median(per_gal_obj)


def fit_m8c_weighted(
    points: list[dict[str, float | str]], focus_gamma: float, r_tail_kpc: float
) -> tuple[float, float, float]:
    """
    Fit (k, g_dag) for WINNER_V1_M8C using v11 robust train objective.
    Returns: (k_fit, gdag_fit, robust_train_obj)
    """
    def obj(k: float, gd: float) -> float:
        return galaxy_robust_train_objective(points, k, gd, focus_gamma, r_tail_kpc)
...
def main() -> int:
    ...
    for split_seed in split_seeds:
        ...
        # v11 single change: robust per-galaxy aggregation in train objective.
    ...
    manifest = {
        ...
        "fit_objective": {
            ...
            "grid_selection_objective": "winner_v1_weighted_lowacc_outer_galaxy_robust",
            "weight_formula": "1 + focus_gamma * (r/(r+r_tail_kpc)) * 1/sqrt(1 + g_bar/a0)",
            "aggregation": "median over galaxies of weighted chi2 per point (weight-normalized)",
    ...
    report = [
        "# D4 Winner V11 Strict Report",
        "",
        f"- generated_utc: `{manifest['generated_utc']}`",
        f"- winner_formula_id: `{WINNER_FORMULA_ID}`",
        f"- candidate: `{args.candidate}`",
        f"- train objective (single v11 change): `{manifest['fit_objective']['grid_selection_objective']}`",

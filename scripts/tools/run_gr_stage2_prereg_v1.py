#!/usr/bin/env python3
"""
GR Stage-2 prereg runner (v1).

Purpose:
- Freeze and execute a Stage-2 GR expansion protocol with one summary CSV.
- Keep Stage-2 separate from the Stage-1 official GR decision lane.

Scope in Stage-2:
- 3+1/ADM consistency lane: G10 + G11
- strong-field-like radial lane: G12
- tensor-mode lane: G7
...
DEFAULT_OUT_SMOKE = ROOT / "05_validation" / "evidence" / "artifacts" / "gr-stage2-smoke-v1"
DEFAULT_OUT_PREREG = ROOT / "05_validation" / "evidence" / "artifacts" / "gr-stage2-prereg-v1"
...
GATE_SPECS: tuple[GateRunSpec, ...] = (
    GateRunSpec(
        ...
        extra_args=("--phi-scale", f"{PREREG_PHI_SCALE:.2f}"),
    ),
    GateRunSpec(
        key="g11",
    ...
    GateRunSpec(
        ...
        extra_args=(
            "--n-steps",
            str(PREREG_TENSOR_N_STEPS),
            "--dt",
            f"{PREREG_TENSOR_DT:.2f}",
            "--c-wave",
            f"{PREREG_TENSOR_C_WAVE:.2f}",
        ),
    ),
)
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Run GR Stage-2 prereg protocol (v1).")
...
def build_profiles(args: argparse.Namespace) -> list[Profile]:
    if args.mode == "smoke":
        return [Profile("DS-002", PREREG_SEED_START), Profile("DS-003", PREREG_SEED_START), Profile("DS-006", PREREG_SEED_START)]
...
def main() -> int:
    ...
    log("=" * 72)
    log(f"GR Stage-2 prereg v1 | mode={args.mode} | profiles={len(profiles)}")
    ...
    for idx, profile in enumerate(profiles, start=1):
        ...
        lane_3p1 = "pass" if (g10["final"] == "pass" and g11["final"] == "pass") else "fail"
        lane_strong_field = g12["final"]
        lane_tensor = g7["final"]
        stage2_status = (
            "pass"
            if (lane_3p1 == "pass" and lane_strong_field == "pass" and lane_tensor == "pass")
            else "fail"
        )
        ...
        row: dict[str, Any] = {
            ...
            "seed": profile.seed,
            "phi_scale_where_applicable": f"{PREREG_PHI_SCALE:.2f}",
    ...
    stage2_pass = sum(1 for r in summary_rows if r["stage2_status"] == "pass")
    stage2_total = len(summary_rows)

    report_lines = [
        "# GR Stage-2 Prereg Report (v1)",
        "",
        f"- mode: `{args.mode}`",
        f"- generated_utc: `{datetime.utcnow().isoformat()}Z`",
        f"- profiles: `{stage2_total}`",
        f"- stage2_pass: `{stage2_pass}/{stage2_total}`",
        f"- lane_3p1_pass: `{sum(1 for r in summary_rows if r['lane_3p1_status'] == 'pass')}/{stage2_total}`",
        f"- lane_strong_field_pass: `{sum(1 for r in summary_rows if r['lane_strong_field_status'] == 'pass')}/{stage2_total}`",
        f"- lane_tensor_pass: `{sum(1 for r in summary_rows if r['lane_tensor_status'] == 'pass')}/{stage2_total}`",
        "",
        "## Notes",
        "",
        "- Stage-2 is candidate expansion lane; does not overwrite Stage-1 official GR decision gate.",
        "- No threshold or formula changes were applied in gate runner scripts.",
        "- All thresholds are inherited from gate scripts G10, G11, G12, G7.",
        "",
    ]
    ...
    manifest = {
        ...
        "phi_scale_where_applicable": PREREG_PHI_SCALE,
        "tensor_config": {
            "n_steps": PREREG_TENSOR_N_STEPS,
            "dt": PREREG_TENSOR_DT,
            "c_wave": PREREG_TENSOR_C_WAVE,
        },
        "stage2_lanes": {
            "lane_3p1": ["G10", "G11"],
        ...
        "notes": [
            "Stage-2 remains prereg/candidate until promotion criteria are explicitly closed.",
            "QM lane is tracked separately and excluded from Stage-2 decision CSV.",
        ],
    }

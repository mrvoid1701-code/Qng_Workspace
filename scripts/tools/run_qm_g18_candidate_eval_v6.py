#!/usr/bin/env python3
"""
Run G18 candidate-v6 multi-scale basin + multi-window peak-envelope evaluation.
...
- update G18d only:
  ...
  - if G18d-v1 fails -> evaluate local spectral dimension on two peak basins
    using fixed multi-window diffusion-fit set over fixed basin quantile set
  - use the peak-envelope aggregate over all local basin scales
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Run QM G18 candidate-v6 multi-scale peak-envelope evaluation.")
    ...
    p.add_argument("--basin-quantiles", default="0.15,0.22")
...
def parse_quantiles(raw: str, fallback_q: float) -> list[float]:
    vals: list[float] = []
    for tok in (raw or "").split(","):
        part = tok.strip()
        if not part:
        ...
        try:
            ...
            continue
        q = max(0.05, min(0.40, q))
        vals.append(q)
    if not vals:
        return [max(0.05, min(0.40, fallback_q))]
    out: list[float] = []
    for q in vals:
        if q not in out:
            out.append(q)
...
def main() -> int:
    ...
    quantiles = parse_quantiles(args.basin_quantiles, args.basin_quantile)
    ...
    for srow in source_rows:
        ...
        if source_g18 == "pass":
            ...
            best_peak_1: dict[str, Any] | None = None
            best_peak_2: dict[str, Any] | None = None
            for q in quantiles:
                ...
                stats_1 = compute_local_ds_multiwindow(
                    neighbours=local1,
                    seed=seed,
                    rng_offset=args.rng_offset + 1 + int(round(1000.0 * q)),
                ...
                stats_2 = compute_local_ds_multiwindow(
                    neighbours=local2,
                    seed=seed,
                    rng_offset=args.rng_offset + 2 + int(round(1000.0 * q)),
                    n_walks=local_walks_eff,
                    n_steps=args.local_steps,
                    windows=windows,
                    min_r2=args.min_window_r2,
                ...
                if ds1 is not None:
                    cand1 = {
                        "ds": float(ds1),
                        "r2": float(stats_1["best_r2"]) if stats_1["best_r2"] is not None else -1.0,
                        "nfit": int(stats_1["best_nfit"]),
                        "win": str(stats_1["best_window"]),
                        "q": float(q),
                        "n": len(local1),
                    }
                    if (best_peak_1 is None) or (
                        (cand1["ds"], cand1["r2"], cand1["nfit"]) > (best_peak_1["ds"], best_peak_1["r2"], best_peak_1["nfit"])
                    ):
                        best_peak_1 = cand1
                ...
                if ds2 is not None:
                    cand2 = {
                        "ds": float(ds2),
                        "r2": float(stats_2["best_r2"]) if stats_2["best_r2"] is not None else -1.0,
                        "nfit": int(stats_2["best_nfit"]),
                        "win": str(stats_2["best_window"]),
                        "q": float(q),
                        "n": len(local2),
                    }
                    if (best_peak_2 is None) or (
                        (cand2["ds"], cand2["r2"], cand2["nfit"]) > (best_peak_2["ds"], best_peak_2["r2"], best_peak_2["nfit"])
                    ):
                        best_peak_2 = cand2

            if best_peak_1 is not None:
                local_ds_1 = float(best_peak_1["ds"])
                local_r2_1 = float(best_peak_1["r2"])
                nfit_1 = int(best_peak_1["nfit"])
                win_1 = str(best_peak_1["win"])
                qsel_1 = float(best_peak_1["q"])
                basin_n1 = int(best_peak_1["n"])
            if best_peak_2 is not None:
                local_ds_2 = float(best_peak_2["ds"])
                local_r2_2 = float(best_peak_2["r2"])
                nfit_2 = int(best_peak_2["nfit"])
                win_2 = str(best_peak_2["win"])
                qsel_2 = float(best_peak_2["q"])
                basin_n2 = int(best_peak_2["n"])
            ...
            if vals:
                ...
                if ds_lo < ds_local_peak_envelope < ds_hi:
                    g18d_v2 = "pass"
                    g18d_rule = "local_ds_multiscale_multiwindow_peak_envelope_recovery"
                else:
                    g18d_v2 = "fail"
                    g18d_rule = "local_ds_multiscale_multiwindow_peak_envelope_retain_fail"
            else:
                ds_local_agg = None
                ds_local_peak_envelope = None
                g18d_v2 = "fail"
                g18d_rule = "local_ds_multiscale_multiwindow_unavailable"
        if source_g18 == "pass":
            ds_local_peak_envelope = None
        ...
        out_rows.append(
            {
                ...
                "g18d_local_best_quantile_peak1": qsel_1,
                "g18d_local_best_quantile_peak2": qsel_2,
    ...
    report_lines = [
        ...
        "## Pass Counts",
        "",
        f"- G18 v1 -> v6: `{g18_v1_pass}/{n} -> {g18_v2_pass}/{n}`",
        f"- QM lane v1 -> v6: `{lane_v1_pass}/{n} -> {lane_v2_pass}/{n}`",
    ...
    manifest = {
        ...
        "profiles": n,
        "policy_id": "qm-g18-candidate-v6-local-ds-multiscale-multiwindow-peak-envelope",
        "mix_ratio_threshold": args.mix_ratio_threshold,
        "mix_distance_threshold": args.mix_distance_threshold,
        ...
        "notes": [
            ...
            "Candidate is governance-layer fixed multi-scale basin + multi-window local-ds peak-envelope recovery.",
        ],
    }

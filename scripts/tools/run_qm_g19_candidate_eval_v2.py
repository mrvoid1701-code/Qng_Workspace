#!/usr/bin/env python3
"""
Run G19 candidate-v2 high-signal slope evaluation on frozen QM summaries.
...
- for source G19 fail rows:
  ...
  - recompute G19d with high-signal slope envelope on thermal propagator pairs
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Run G19 candidate-v2 high-signal slope evaluation.")
    p.add_argument("--source-summary-csv", required=True)
    p.add_argument("--out-dir", default=str(DEFAULT_OUT))
    p.add_argument("--high-signal-quantiles", default="0.20,0.30")
...
def main() -> int:
    ...
    for srow in source_rows:
        ...
        if source_g19 == "pass":
            ...
            if base["g19a_status"] == "pass" and base["g19b_status"] == "pass" and base["g19c_status"] == "pass":
                slope_best = hs["slope_best"]
                if slope_best is not None and pass_threshold(float(slope_best), base["g19d_threshold_cmp"], base["g19d_threshold"]):
                    g19d_v2 = "pass"
                    g19d_rule = "high_signal_slope_envelope_recovery"
                else:
                    g19d_v2 = "fail"
    ...
    manifest = {
        ...
        "profiles": n,
        "policy_id": "qm-g19-candidate-v2-high-signal-envelope",
        "high_signal_quantiles": quantiles,
        "min_support": int(args.min_support),
        "notes": [
            "Candidate-only G19d high-signal slope envelope recovery.",

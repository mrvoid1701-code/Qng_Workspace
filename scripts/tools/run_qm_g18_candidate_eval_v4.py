#!/usr/bin/env python3
"""
Run G18 candidate-v4 peak-envelope evaluation on frozen QM Stage-1 summaries.
...
- update G18d only:
  ...
  - if G18d-v1 fails -> evaluate local spectral dimension on two peak basins
    and use peak-envelope aggregate (`max(local_ds_peak1, local_ds_peak2)`)
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Run QM G18 candidate-v4 peak-envelope evaluation.")
...
def main() -> int:
        ...
        if g18d_v1 == "pass":
            ...
            if vals:
                ...
                if ds_lo < ds_local_peak_envelope < ds_hi:
                    g18d_v2 = "pass"
                    g18d_rule = "local_ds_peak_envelope_recovery"
                else:
                    g18d_v2 = "fail"
                    g18d_rule = "local_ds_peak_envelope_retain_fail"
            else:
                ds_local_agg = None
    ...
    report_lines = [
        ...
        "## Pass Counts",
        "",
        f"- G18 v1 -> v4: `{g18_v1_pass}/{n} -> {g18_v2_pass}/{n}`",
        f"- QM lane v1 -> v4: `{lane_v1_pass}/{n} -> {lane_v2_pass}/{n}`",
    ...
    manifest = {
        ...
        "profiles": n,
        "policy_id": "qm-g18-candidate-v4-local-ds-peak-envelope",
        "mix_ratio_threshold": args.mix_ratio_threshold,
        "mix_distance_threshold": args.mix_distance_threshold,
        ...
        "notes": [
            ...
            "Candidate is governance-layer local-ds peak-envelope recovery.",
        ],
    }

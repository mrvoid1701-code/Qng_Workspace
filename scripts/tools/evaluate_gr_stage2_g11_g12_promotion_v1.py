#!/usr/bin/env python3
"""
Evaluate promotion criteria for GR Stage-2 G11/G12 candidate v2 package.

This evaluator applies preregistered acceptance checks only.
No gate math is modified.
"""
...
DEFAULT_OUT = (
    ...
    / "evidence"
    / "artifacts"
    / "gr-stage2-g11-g12-promotion-eval-v1"
)


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Evaluate GR Stage-2 G11/G12 candidate promotion criteria.")
    p.add_argument("--summary-csv", required=True)
    p.add_argument("--out-dir", default=str(DEFAULT_OUT))
    p.add_argument("--eval-id", default="gr-stage2-g11-g12-promotion-v1")
...
def main() -> int:
    ...
    stage2_eval = evaluate_gate(
        rows=rows,
        gate_name="STAGE2",
        v1_key="stage2_v1_status",
        v2_key="stage2_v2_status",
        require_zero_degraded=args.require_zero_degraded,
        require_net_uplift=args.require_net_uplift,
        require_per_dataset_nondegrade=args.require_per_dataset_nondegrade,
    )

    overall_pass = bool(g11_eval["decision"] and g12_eval["decision"] and stage2_eval["decision"])

    ds_rows = g11_eval["dataset_rows"] + g12_eval["dataset_rows"] + stage2_eval["dataset_rows"]
    ds_csv = out_dir / "dataset_summary.csv"
    write_csv(ds_csv, ds_rows, list(ds_rows[0].keys()) if ds_rows else ["gate", "dataset_id"])
    ...
    report = {
        ...
        "stage2": {
            "totals": totals_json(stage2_eval),
            "checks": stage2_eval["checks"],
            "decision": bool(stage2_eval["decision"]),
        },
    ...
    lines: list[str] = []
    lines.append("# GR Stage-2 G11/G12 Candidate Promotion Evaluation (v1)")
    lines.append("")
    ...
    lines.append("")
    for name, ev in [("G11", g11_eval), ("G12", g12_eval), ("STAGE2", stage2_eval)]:
        t: Totals = ev["totals"]
    ...
    print(f"dataset_summary_csv: {ds_csv}")

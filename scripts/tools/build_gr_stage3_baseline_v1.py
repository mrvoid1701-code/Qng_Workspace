#!/usr/bin/env python3
"""Build Stage-3 regression baseline JSON from Stage-3 official summary."""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Build Stage-3 baseline JSON from official summary.csv.")
    p.add_argument("--summary-csv", required=True)
    p.add_argument("--out-json", required=True)
    p.add_argument("--baseline-id", default="gr-stage3-official-baseline-v1")
...
def main() -> int:
    ...
    baseline = {
        ...
        "notes": [
            "Generated from Stage-3 official policy summary.",
            "Status-only guard baseline for Stage-3 governance mapping.",
        ],
        "profiles": profiles,
        "compare": {

#!/usr/bin/env python3
"""
Evaluate QM G17 candidate-v2 promotion criteria (v1).
...
DEFAULT_OUT = ROOT / "05_validation" / "evidence" / "artifacts" / "qm-g17-promotion-eval-v1"


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Evaluate QM G17 candidate-v2 promotion criteria.")
    p.add_argument("--summary-csv", required=True)
    p.add_argument("--out-dir", default=str(DEFAULT_OUT))
    p.add_argument("--eval-id", default="qm-g17-promotion-v1")
...
def main() -> int:
    ...
    g17_tot = summarize(eval_rows, "g17_status_v1", "g17_status_v2")
    lane_tot = summarize(eval_rows, "all_pass_qm_lane_v1", "all_pass_qm_lane_v2")

    g17_ds = per_dataset_summary(eval_rows, "g17_status_v1", "g17_status_v2", "G17")
    lane_ds = per_dataset_summary(eval_rows, "all_pass_qm_lane_v1", "all_pass_qm_lane_v2", "QM_LANE")
    ds_rows = g17_ds + lane_ds
    write_csv(
        out_dir / "dataset_summary.csv",
        ds_rows,
        ["scope", "dataset_id", "n", "v1_pass", "v2_pass", "improved", "degraded", "uplift_pp", "failcase_uplift_pp", "nondegrade_pass"],
    )

    checks_g17 = checks_for(g17_tot, g17_ds, args)
    checks_lane = checks_for(lane_tot, lane_ds, args)
    decision_g17 = all(checks_g17.values())
    decision_lane = all(checks_lane.values())
    overall = decision_g17 and decision_lane
    ...
    report_json = {
        ...
        "g17": {
            "totals": {
                "n": g17_tot.n,
                "v1_pass": g17_tot.v1_pass,
                "v2_pass": g17_tot.v2_pass,
                "improved": g17_tot.improved,
                "degraded": g17_tot.degraded,
                "uplift_pp": round(g17_tot.uplift_pp, 6),
                "failcase_uplift_pp": round(g17_tot.failcase_uplift_pp, 6),
            },
            "checks": checks_g17,
            "decision": decision_g17,
        },
        "qm_lane": {
            "totals": {
                "n": lane_tot.n,
    ...
    lines = [
        "# QM G17 Candidate-v2 Promotion Eval",
        "",
        ...
        "## Scope Decisions",
        "",
        f"- G17: decision={'PASS' if decision_g17 else 'HOLD'}, v1_pass={g17_tot.v1_pass}, v2_pass={g17_tot.v2_pass}, improved={g17_tot.improved}, degraded={g17_tot.degraded}, uplift_pp={g17_tot.uplift_pp:.3f}, failcase_uplift_pp={g17_tot.failcase_uplift_pp:.3f}",
        f"  checks: zero_degraded={bool_str(checks_g17['zero_degraded'])}, net_uplift_failcases={bool_str(checks_g17['net_uplift_failcases'])}, per_dataset_nondegrade={bool_str(checks_g17['per_dataset_nondegrade'])}",

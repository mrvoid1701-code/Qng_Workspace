#!/usr/bin/env python3
"""
Run G18 candidate-v2 hybrid evaluation on frozen QM Stage-1 official summaries.
...
- update G18d only:
  ...
  - if G18d-v1 fails and profile is multi-peak -> evaluate local spectral dimension
    on per-peak local basins and use robust aggregate (median)
  - apply same G18d threshold band as v1 (default 1.2..3.5, parsed from metric file if available)
  - otherwise keep fail
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Run QM G18 candidate-v2 hybrid evaluation.")
    ...
    p.add_argument("--rng-offset", type=int, default=17001)
...
def parse_g18_metric_checks(path: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "g18a_value": to_float(gate("G18A").get("value", "nan"), float("nan")),
        "g18b_value": to_float(gate("G18B").get("value", "nan"), float("nan")),
        "g18c_value": to_float(gate("G18C").get("value", "nan"), float("nan")),
...
def main() -> int:
    ...
    for srow in source_rows:
        ...
        if g18d_v1 == "pass":
            ...
            local_ds_1, local_r2_1, nfit_1 = compute_local_ds(
                neighbours=local1,
                seed=seed,
                rng_offset=args.rng_offset + 1,
                n_walks=args.local_walks,
                n_steps=args.local_steps,
                t_lo=args.local_t_lo,
                t_hi=args.local_t_hi,
            )
            local_ds_2, local_r2_2, nfit_2 = compute_local_ds(
                neighbours=local2,
                seed=seed,
            ...
            if vals:
                ...
                if ds_lo < ds_local_agg < ds_hi:
                    g18d_v2 = "pass"
                    g18d_rule = "local_ds_recovery"
                else:
                    g18d_v2 = "fail"
                    g18d_rule = "local_ds_retain_fail"
            else:
                ds_local_agg = None
                g18d_v2 = "fail"
                g18d_rule = "local_ds_unavailable"
        else:
            ds_local_agg = None
            g18d_v2 = "fail"
            g18d_rule = "retain_v1_fail_non_multipk"
        ...
        out_rows.append(
            {
                ...
                "g17_status": g17,
                "g18_status_v1": base["g18_status_v1"],
    ...
    report_lines = [
        ...
        "## Notes",
        "",
        "- G18d-v1 is preserved in single-peak/non-multi-peak cases.",
        "- G18d-v2 applies local spectral-dimension recovery only under multi-peak mixing.",
    ...
    manifest = {
        ...
        "profiles": n,
        "policy_id": "qm-g18-candidate-v2-hybrid-local-ds",
        "mix_ratio_threshold": args.mix_ratio_threshold,
        "mix_distance_threshold": args.mix_distance_threshold,
        ...
        "notes": [
            ...
            "Candidate is governance-layer hybrid for multi-peak robustness.",
        ],
    }

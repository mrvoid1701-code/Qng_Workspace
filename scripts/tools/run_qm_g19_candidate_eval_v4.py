#!/usr/bin/env python3
"""
Run G19 candidate-v4 hybrid slope evaluation on frozen QM summaries.
...
- for source G19 fail rows:
  ...
  - first recovery path: high-signal median slope (v3 path)
  - second recovery path (multi-peak only): local-window slope best-of-fixed windows
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Run G19 candidate-v4 hybrid slope evaluation.")
    ...
    p.add_argument("--local-window-fracs", default="0.03,0.05,0.08,0.10")
    p.add_argument("--local-window-min-support", type=int, default=20)
    p.add_argument("--local-window-stride-frac", type=float, default=0.01)
    p.add_argument("--enable-local-window-on-multipeak-only", default="true")
...
def local_window_best_slope(
    ...
    n_total = len(pairs)
    stride_n = max(1, int(round(max(0.001, stride_frac) * n_total)))

    best_slope: float | None = None
    best_frac = ""
    best_n = 0
    best_r_lo: float | None = None
    best_r_hi: float | None = None
    n_windows = 0

    for frac in window_fracs:
        n_used = max(min_support, int(round(frac * n_total)))
        n_used = min(n_used, n_total)
        if n_used < 2:
            continue
        start = 0
        while start + n_used <= n_total:
            seg = pairs[start : start + n_used]
            sl = ols_slope([v[0] for v in seg], [v[1] for v in seg])
            n_windows += 1
            if sl is not None and (best_slope is None or sl < best_slope):
                best_slope = sl
                best_frac = f"q{int(round(100.0 * frac)):02d}"
                best_n = n_used
                best_r_lo = seg[0][0]
                best_r_hi = seg[-1][0]
            start += stride_n

    out["best_slope"] = best_slope
    out["best_frac"] = best_frac
    out["best_n"] = best_n
    out["best_r_lo"] = best_r_lo
    out["best_r_hi"] = best_r_hi
    out["n_windows_tested"] = n_windows
    return out
...
def main() -> int:
    ...
    window_fracs = parse_quantiles(args.local_window_fracs)
    local_on_multipeak_only = parse_bool(args.enable_local_window_on_multipeak_only, True)
    ...
    for srow in source_rows:
        ...
        multi_peak_bool = parse_bool(multi_peak, False)
        ...
        lw = local_window_best_slope(
            prop_csv,
            window_fracs=window_fracs,
            min_support=int(args.local_window_min_support),
            stride_frac=float(args.local_window_stride_frac),
        ) if prop_csv.exists() else {
            "global_n": 0,
            "best_slope": None,
            "best_frac": "",
            "best_n": 0,
        ...
        if source_g19 == "pass":
            ...
            if base["g19a_status"] == "pass" and base["g19b_status"] == "pass" and base["g19c_status"] == "pass":
                ...
                elif (not local_on_multipeak_only or multi_peak_bool) and lw["best_slope"] is not None and pass_threshold(
                    float(lw["best_slope"]), base["g19d_threshold_cmp"], base["g19d_threshold"]
                ):
                    g19d_v2 = "pass"
                    g19d_rule = "local_window_best_recovery"
                else:
                    g19d_v2 = "fail"
        ...
        rec: dict[str, Any] = {
            ...
            "g19d_slope_local_window_best": ""
            if lw["best_slope"] is None
            else f"{float(lw['best_slope']):.12e}",
            "g19d_slope_local_window_best_frac": str(lw["best_frac"]),
            "g19d_slope_local_window_best_n": int(lw["best_n"]),
            "g19d_slope_local_window_best_r_lo": ""
            if lw["best_r_lo"] is None
            else f"{float(lw['best_r_lo']):.9f}",
            "g19d_slope_local_window_best_r_hi": ""
            if lw["best_r_hi"] is None
            else f"{float(lw['best_r_hi']):.9f}",
            "g19d_slope_local_window_windows_tested": int(lw["n_windows_tested"]),
    ...
    manifest = {
        ...
        "profiles": n,
        "policy_id": "qm-g19-candidate-v4-hybrid-local-window",
        "high_signal_quantiles": quantiles,
        "min_support": int(args.min_support),
        "local_window_fracs": window_fracs,
        "local_window_min_support": int(args.local_window_min_support),
        "local_window_stride_frac": float(args.local_window_stride_frac),
        "enable_local_window_on_multipeak_only": bool(local_on_multipeak_only),
        "notes": [
            "Candidate-only G19d hybrid recovery: high-signal median + local-window best in multi-peak.",

#!/usr/bin/env python3
"""
Run G17b candidate-v6 evaluation on frozen QM Stage-1 official summaries.
...
- for G17b-v1 fail cases:
  - compute high-signal OLS slope across a fixed quantile grid of |G_ij|
  - aggregate with median slope (robust to tail contamination)
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Run G17b candidate-v6 high-signal evaluation.")
...
def high_signal_slope_median(
    sample_csv: Path, quantiles: list[float], min_support: int
    ...
    for q in quantiles:
        ...
        n_used = min(n_used, n)
        sel = pairs[:n_used]
        s = ols_slope([p[1] for p in sel], [p[2] for p in sel])
        if s is None:
            continue
        slopes.append(s)
        if n_used > max_used:
            max_used = n_used
    if not slopes:
        return None, max_used, 0
    return float(statistics.median(slopes)), max_used, len(slopes)
        ...
        out.append(
            {
                ...
                "g17_v6_pass": sum(1 for r in sub if r["g17_status_v6"] == "pass"),
                "g17b_v1_pass": sum(1 for r in sub if r["g17b_status_v1"] == "pass"),
                "g17b_v6_pass": sum(1 for r in sub if r["g17b_status_v6"] == "pass"),
                "qm_lane_v1_pass": sum(1 for r in sub if r["all_pass_qm_lane_v1"] == "pass"),
                "qm_lane_v6_pass": sum(1 for r in sub if r["all_pass_qm_lane_v6"] == "pass"),
                "improved_g17": sum(
                    1 for r in sub if r["g17_status_v1"] == "fail" and r["g17_status_v6"] == "pass"
                ),
                "degraded_g17": sum(
                    1 for r in sub if r["g17_status_v1"] == "pass" and r["g17_status_v6"] == "fail"
                ),
                "improved_qm_lane": sum(
                    1
                    for r in sub
                    if r["all_pass_qm_lane_v1"] == "fail" and r["all_pass_qm_lane_v6"] == "pass"
                ),
                "degraded_qm_lane": sum(
                    1
                    for r in sub
                    if r["all_pass_qm_lane_v1"] == "pass" and r["all_pass_qm_lane_v6"] == "fail"
                ),
            }
        )
    return out
...
def main() -> int:
    ...
    for srow in source_rows:
        ...
        slope_hs, n_used, n_windows = high_signal_slope_median(
        ...
        if g17b_v1 == "pass":
            ...
            if slope_hs is not None and n_used >= args.min_support and slope_hs < g17b_thr:
                g17b_v6 = "pass"
                g17b_rule = "high_signal_median_recovery"
            else:
                g17b_v6 = "fail"
        ...
        g17_v6 = "pass" if (g17a_v2 == "pass" and g17b_v6 == "pass" and g17c == "pass" and g17d == "pass") else "fail"
        ...
        lane_v6 = "pass" if (g17_v6 == "pass" and g18 == "pass" and g19 == "pass" and g20 == "pass") else "fail"
        ...
        out_rows.append(
            {
                ...
                "all_pass_qm_lane_v2": lane_v6,
                "all_pass_qm_lane_v6": lane_v6,
    ...
    g17_v6_pass = sum(1 for r in out_rows if r["g17_status_v6"] == "pass")
    lane_v1_pass = sum(1 for r in out_rows if r["all_pass_qm_lane_v1"] == "pass")
    lane_v6_pass = sum(1 for r in out_rows if r["all_pass_qm_lane_v6"] == "pass")
    g17b_v1_pass = sum(1 for r in out_rows if r["g17b_status_v1"] == "pass")
    g17b_v6_pass = sum(1 for r in out_rows if r["g17b_status_v6"] == "pass")
    improved_g17 = sum(1 for r in out_rows if r["g17_status_v1"] == "fail" and r["g17_status_v6"] == "pass")
    degraded_g17 = sum(1 for r in out_rows if r["g17_status_v1"] == "pass" and r["g17_status_v6"] == "fail")
    improved_lane = sum(
        1 for r in out_rows if r["all_pass_qm_lane_v1"] == "fail" and r["all_pass_qm_lane_v6"] == "pass"
    )
    degraded_lane = sum(
        1 for r in out_rows if r["all_pass_qm_lane_v1"] == "pass" and r["all_pass_qm_lane_v6"] == "fail"
    )
    ...
    lines = [
        ...
        "## Pass Counts",
        "",
        f"- G17b v1 -> v6: `{g17b_v1_pass}/{n} -> {g17b_v6_pass}/{n}`",
        f"- G17 v1 -> v6: `{g17_v1_pass}/{n} -> {g17_v6_pass}/{n}`",
        f"- QM lane v1 -> v6: `{lane_v1_pass}/{n} -> {lane_v6_pass}/{n}`",
    ...
    manifest = {
        ...
        "profiles": n,
        "policy_id": "qm-g17b-candidate-v6-high-signal-median",
        "high_signal_quantiles": quantiles,
        "min_support": args.min_support,
        "fallback_threshold": args.fallback_threshold,
        "notes": [
            "Candidate-only G17b high-signal median slope recovery on fail cases.",
